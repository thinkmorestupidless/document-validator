...
object Worker {
  ...
  case class DeliveredMessage(confirmTo: ActorRef[ConsumerController.Confirmed], message: WorkManager.WorkerCommand, seqNr: Long)
      extends Message
  case class WorkComplete(result: String) extends Message
  case class WorkTimeout()                extends Message

  def apply(workerId: String = UUID.randomUUID().toString,
    ...
    Behaviors.setup[Message] { ctx =>
      ...
      val deliverAdapter =
        ctx.messageAdapter[ConsumerController.Delivery[WorkManager.WorkerCommand]](d => DeliveredMessage(d.confirmTo, d.message, d.seqNr))
...
class Worker private (workerId: String, ctx: ActorContext[Worker.Message], workExecutorFactory: () => Behavior[ExecuteWork]) {
  ...
  def working(workId: String,
              workExecutor: ActorRef[ExecuteWork],
